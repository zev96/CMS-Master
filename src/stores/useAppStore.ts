...
interface CacheState {
  ...
  assetCache: Map<string, TextAsset | VisualAsset>;
  chartCache: Map<string, SavedChart>;
}

interface AppState extends Database, UIState {
  // 性能监控
  ...
  // 模板页面状态
  currentTemplateView: 'list' | 'detail' | 'form';
  selectedTemplateId: string | null;
  editingTemplateId: string | null;

  // 编辑器状态
  currentTemplateInEditor: Template | null;

  // 标记模板内容是否已被用户修改
  templateContentModified: boolean;

  // 当前编辑内容状态
  currentEditorContent: string;

  // 编辑器历史记录
  editorHistory: Array<{ content: string; timestamp: string }>;
  ...
  actions: {
    ...
    showProductForm: (productId?: string) => void;

    // 模板页面导航操作
    showTemplateList: () => void;
    showTemplateDetail: (templateId: string) => void;
    showTemplateForm: (templateId?: string) => void;
    ...
    useTemplate: (templateId: string) => boolean;

    // 素材操作
    addTextAsset: (asset: Omit<TextAsset, 'id' | 'created_at' | 'updated_at'>) => Promise<void>;
    updateTextAsset: (id: string, asset: Partial<TextAsset>) => Promise<void>;
    deleteTextAsset: (id: string) => Promise<void>;

    addVisualAsset: (asset: Omit<VisualAsset, 'id' | 'created_at' | 'updated_at'>) => Promise<void>;
    updateVisualAsset: (id: string, asset: Partial<VisualAsset>) => Promise<void>;
    deleteVisualAsset: (id: string) => Promise<void>;

    // 文章操作
    addArticle: (article: Omit<ArticleContent, 'id' | 'created_at' | 'updated_at'>) => void;
    updateArticle: (id: string, article: Partial<ArticleContent>) => void;
    deleteArticle: (id: string) => void;
    getCurrentArticle: () => ArticleContent | undefined;

    // 素材使用操作
    insertAssetToEditor: (asset: TextAsset | VisualAsset) => void;

    // 图表操作
    addChart: (chart: Omit<SavedChart, 'id' | 'createdAt'>) => Promise<void>;
    updateChart: (id: string, chart: Partial<SavedChart>) => Promise<void>;
    ...
    resetTemplateModifiedStatus: () => void;
    isTemplateModified: () => boolean;
    clearTemplateInEditor: () => void;
    ...
    // 批量操作
    batchUpdateProducts: (updates: Array<{ id: string; data: ProductFormData }>) => Promise<void>;
    batchDeleteProducts: (ids: string[]) => Promise<void>;

    // 搜索和筛选优化
    searchProducts: (query: string) => Product[];
    filterProducts: (filters: Record<string, any>) => Product[];
    ...
    // 清理损坏的图表数据
    cleanupChartData: () => void;

    // 图表插入到编辑器
    insertChartToEditor: (chart: SavedChart) => void;

    // 添加编辑器历史记录相关方法
    saveEditorHistory: (content: string) => void;
    getEditorHistory: () => Array<{ content: string; timestamp: string }>;
  };
}
      ...
      const actions: AppState['actions'] = {
        ...
        addArticle: article =>
          set(state => {
            ...
            state.articles.push(newArticle as any);
          }),
        ...
        // 添加编辑器历史记录相关方法
        saveEditorHistory: (content: string) => {
