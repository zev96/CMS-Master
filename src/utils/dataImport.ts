...
interface ExcelColumnMapping {
  ...
  category: string;
  description: string;
  [key: string]: string; // 允许动态参数映射
}

const defaultColumnMapping: ExcelColumnMapping = {
  brand: '品牌',
  modelName: '型号',
  price: '价格',
  category: '品类',
...
// 工具函数：解析产品品类
const parseCategory = (value: any): ProductCategory => {
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = e => {
      try {
        ...
        const parameterColumns: { [key: string]: number } = {};
...
export const generateSampleProducts = (): Product[] => {
  const productData = [
    // 吸尘器类别
    {
      ...
      category: '吸尘器' as ProductCategory,
      price: 1299,
    ...
    // 宠物空气净化器类别
    {
      ...
      category: '宠物空气净化器' as ProductCategory,
      price: 2199,
    ...
    },
    {
      ...
      category: '空气净化器' as ProductCategory,
      price: 899,
    ...
    // 猫粮类别
    {
      ...
      category: '猫粮' as ProductCategory,
      price: 168,
...
// 生成示例模板数据
export const generateSampleTemplates = (): Template[] => {
  const templates = [
    {
      title: '产品评测模板',
...
export const generateSampleAssets = () => {
  const textAssets: TextAssetLibrary = {
    general: {
      introduction: [
        {
          id: 'intro-1',
