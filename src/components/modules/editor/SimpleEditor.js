...
// 提取Markdown的辅助函数
const extractMarkdownFromHtml = html => {
  let markdown = html;
...
const SimpleEditor = React.forwardRef(({ content, onChange }, ref) => {
  ...
  // 全局图表插入函数
  useEffect(() => {
    window.insertChartToContentEditor = (chartUrl, title, chartType) => {
      let newContent = '';
      if (chartType === 'table') {
        // 对于表格类型，直接插入HTML内容
    ...
    return () => {
      delete window.insertChartToContentEditor;
    };
  }, [content, onChange]);
  const handleFocus = () => {
    setIsEditing(true);
